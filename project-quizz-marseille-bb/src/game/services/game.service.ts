...
export class GameService implements IGameService {
  ...
  async create(
    ...
    const gameQuestions =
      await this.questionService.initizializeQuestions(quizId);
  ...
  async showFirstQuestion(gameId: string, question: any): Promise<void> {
    this.websocketService
      .getServer()
      .to(gameId)
      .emit(WebSocketEvents.SHOW_NEXT_QUESTION, question);
  }
  ...
  async showNextQuestion(gameId: string): Promise<void> {
    ...
    const question =
      await this.questionService.getQuestionByIndexAssociateWithChoice(
    ...
    this.websocketService
      .getServer()
      .to(gameId)
      .emit(WebSocketEvents.SHOW_SCORE, scoreInstance);
  ...
  async findGameWithQuizById(
    gameId: string,
  ): Promise<(Game & { quiz: { maxPlayers: number } }) | null> {
    return this.database.game.findUnique({
      where: { id: gameId },
      include: {
        quiz: {
